...
for k in range(0,Nsteps):
 ...
 print('time step',k)   
 if (k==0):
   for ens in range (0,Nens): # sample as much as Nens(20)
    tt =  torch.randint(0, T, (1,), device=device).long() #random time? why?
    ...
    # T ---> T-1
    u=x_noisy - model(x_noisy, tt) ## why substract ? (delta_noise?)
