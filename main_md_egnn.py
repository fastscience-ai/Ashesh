...
exp_name = f'egnn_{"{:.2e}".format(learning_rate)}_t{args.timesteps}_{temp_}K_eps_model_is_noise_feat_wo_pbc_2'
...
model.load_state_dict(torch.load(os.path.join(model_save_path, str(args.num_epochs)+'.pt')))
...
np.savez(os.path.join(args.result_path, './'+exp_name+'_100_with_loop.npz'), pred = pred_denorm, GT = te_y_denorm)
