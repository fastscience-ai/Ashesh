...
exp_name = f'egnn_3lr_3e-4_t250_{args.temperature}K'
...
#data load
#(50000, 64, 9)
x_tensor=np.load(os.path.join(dataset_path, f"input_long_{args.temperature}K.npy"))
s = np.shape(x_tensor)
...
y_tensor=np.load(os.path.join(dataset_path, f"output_long_{args.temperature}K.npy"))
s = np.shape(y_tensor)
...
#%%
# laod weights
model_name = 'Diffusion_MD_trial_'+str(args.exp_name)+'_'+str(num_epochs)+'.pt'
...
np.savez(os.path.join(result_path, './'+'Diffusion_MD_trial_egnn_3lr_3e-4_t250_1000'+'.npz'), pred = pred, GT = te_y)
