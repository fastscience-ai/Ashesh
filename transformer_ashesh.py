...
class Transformer(nn.Module):
    def __init__(self, num_atoms=32, input_size=9, dim=64,
        ...
        self.input_emb = nn.Sequential(
            nn.Linear(2*dim*input_size, dim*input_size*self.in_channels),
            nn.Linear(dim*input_size,dim*input_size*self.in_channels)
        )
    ...
    def forward(self, x, cond,  t):
       ...
       # print(x.shape)
        x = self.input_emb(x)
        x = torch.reshape(x, [-1, self.dim, self.input_size, self.in_channels])
