...
single_temp_{args.temperature}K_T{args.timesteps}_lr{learning_rate}"
        ...
        if (k==0):
            for ens in range (0, Nens):
                if args.how_to_sample == "one_step":
                    ...
                    tt =  torch.tensor([10], device=device).long() # 이제 randn이니까?
                else:
            ...
            for ens in range (0, Nens):
                if args.how_to_sample == "one_step":
                    # tt = torch.randint(0, T, (1,), device=device).long() # diffusion time step--> random tt 
                    tt =  torch.tensor([10], device=device).long()
                else:
                    tt =  torch.tensor([T-1], device=device).long()
                # generate from pure noise : 
                x_noisy = torch.randn_like(x_0)
...
np.savez(os.path.join(args.result_path, f'./{exp_name}_{args.how_to_sample}_10'),pred,te_y_denorm[:Nsteps])
