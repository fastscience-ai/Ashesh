...
from transformer_uncond import *
...
device = "cuda" if torch.cuda.is_available() else "cpu"
model.load_state_dict(torch.load("./results/Diffusion_MD_trial_500000.pt")) # This for unconditional diffusion model
model.to(device)
...
pred = np.zeros([1000,Nsteps,Nens,1,64,9]) # 64 atoms 9 features
for i in range(1000):
    for k in range(0, Nsteps):
        ...
        if (k==0):
            for ens in range (0, Nens):
                ...
                predicted_noisy =  model(x_noisy, tt)
